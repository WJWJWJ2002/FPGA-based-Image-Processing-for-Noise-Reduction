module top(clk, new_pix);
	...
	wire clk, gen_req, done_init_buf, data_valid, rden_rom, done_filt, done_gen;
	wire[DATA_WIDTH-1:0] rom_out_ff, fifo2_out_ff, fifo1_out_ff, row, col;
	reg init_buff;

	fifo_rom memory_buffer (.clk(clk), ,init_buff(init_buff), .gen_req(gen_req), .rden_rom(rden_rom), 
	...
	.done_gen(done_gen), .done_filt(done_filt), .data_valid(data_valid),
