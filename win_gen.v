module win_gen(gen_ready, gen_req, done_gen, done_filt, rden_rom, clk, row1_out_ff, row2_out_ff, rom_out_ff, row, col
	p1, p2, p3, p4, p5, p6, p7, p8, p9
);
	`include "parameters.vh"
	input clk, gen_ready, done_filt; //gen_ready = done_init_buf
	input[DATA_WIDTH-1:0] row1_out_ff, row2_out_ff, rom_out_ff;
	output gen_req, done_gen, rden_rom;
	output[DATA_WIDTH-1:0] p1, p2, p3, p4, p5, p6, p7, p8, p9, row, col;
	wire done_gen, rden_rom;
	reg gen_req;
	reg[1:0] counter;
	reg[2:0] state, next_state;
	reg[DATA_WIDTH-1:0] p1, p2, p3, p4, p5, p6, p7, p8, p9;
	localparam[7:0] MAX_COL = 8'd255, MAX_ROW = 8'd255;
	localparam[2:0] WAIT_INIT = 3'd0, S1 = 3'd1, WAIT = 3'd2, S2 = 3'd3 DONE_GEN = 3'd4, WAIT_FILT = 3'd5;
	
	// Next state combinational logic
	always @(*) begin
		case (state)
			WAIT_INIT: next_state = (gen_ready) ? (S1) : (WAIT_INIT);
			S1: next_state = WAIT;
			WAIT: next_state = (data_valid || (col == 8'd0 && counter == 2'd0) || 
				(col == MAX_COL && counter == 2'd2)) ? (S2) : (WAIT);
			S2: next_state = (counter < 2'd2) ? (WAIT) : (DONE_GEN);
			DONE_GEN: next_state = WAIT_FILT;
			WAIT_FILT: next_state = (done_filt) ? ((col == 8'd0) ? (S1) : (S2)) : (WAIT_FILT);
